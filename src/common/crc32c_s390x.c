...
unsigned int crc32c_be_vx(uint32_t crc, unsigned char const *data, unsigned datalen)
{
	unsigned long prealign, aligned, remaining;

	if(data == 0)
		return crc32c_be(crc, data, datalen);
	...
	if ((unsigned long)data & VX_ALIGN_MASK) {
		...
		datalen -= prealign;
		crc = crc32c_be(crc, data, prealign);
		data = data + prealign;
	}
	...
	crc = crc32c_be_vgfm_16(crc, data, aligned);
	data = data + aligned;

	if (remaining)
		crc = crc32c_be(crc, data, remaining);
